...
namespace Application.Services
{
    public class StudentService : IStudentService
    {
        ...
        {
            var students = await _unitOfWork.StudentRepository.GetAllWithDetailsAsync(); // Updated method
        ...
        {
            var student = await _unitOfWork.StudentRepository.GetByIdWithDetailsAsync(id); // Updated method
            if (student == null)
        ...
        public async Task<ServiceResult<StudentViewDto>> CreateAsync(CreateStudentDto dto) // Changed parameter type
        {
            ...
            if (!string.IsNullOrEmpty(dto.ClassId))
            {
                ...
                if (classEntity.Grade?.CurriculumId != dto.CurriculumId)
                    return ServiceResult<StudentViewDto>.Fail(
                        "Class does not belong to the selected curriculum"
                    );
            }
            ...
            var createdStudent = await _unitOfWork.StudentRepository.GetByIdWithDetailsAsync(
                student.Id
            );
        ...
        public async Task<ServiceResult<StudentViewDto>> UpdateStudentAdditionalInfoAsync(
            string id,
        ...
        )
        {
            ...
            var updatedStudent = await _unitOfWork.StudentRepository.GetByIdWithDetailsAsync(id);
            var viewDto = _mapper.Map<StudentViewDto>(updatedStudent ?? existingStudent);
            return ServiceResult<StudentViewDto>.Ok(
                viewDto,
                "Student additional info updated successfully"
            );
        }

        public async Task<ServiceResult<StudentViewDto>> UpdateAsync(
            string id,
            UpdateStudentDto dto
        ) // Changed parameters
        {
        ...
        public async Task<ServiceResult<IEnumerable<StudentViewDto>>> GetStudentsByCurriculumAsync(
            string curriculumId
        )
        {
            var students = await _unitOfWork.StudentRepository.GetStudentsByCurriculumAsync(
                curriculumId
            );
            if (students == null || !students.Any())
                return ServiceResult<IEnumerable<StudentViewDto>>.Fail(
                    "No students found in this curriculum"
                );
        ...
        public async Task<ServiceResult<StudentViewDto>> MoveStudentToCurriculumAsync(
            string studentId,
            string newCurriculumId
        )
        {
            ...
            var curriculum = await _unitOfWork.CurriculumRepository.GetByIdAsync(newCurriculumId);
            if (curriculum == null)
                return ServiceResult<StudentViewDto>.Fail("Curriculum not found");
            ...
            {
                var classEntity = await _unitOfWork.ClassRepository.GetByIdAsync(student.ClassId);
            ...
            var updatedStudent = await _unitOfWork.StudentRepository.GetByIdWithDetailsAsync(
                studentId
            );
            var viewDto = _mapper.Map<StudentViewDto>(updatedStudent ?? student);
            return ServiceResult<StudentViewDto>.Ok(
                viewDto,
                "Student moved to new curriculum successfully"
            );
        }
        ...
        {
            var count = await _unitOfWork.StudentRepository.GetStudentCountByCurriculumAsync(
                curriculumId
            );
            return ServiceResult<int>.Ok(count, $"Total students in curriculum: {count}");
        }

        public async Task<ServiceResult<List<StudentSearchResultDto>>> SearchStudentsAsync(
            SearchTermDto dto
        )
        {
            try
            {
                if (string.IsNullOrWhiteSpace(dto.SearchTerm) || dto.SearchTerm.Length < 2)
                    return ServiceResult<List<StudentSearchResultDto>>.Ok(
                        new List<StudentSearchResultDto>()
                    );

                var students = await _unitOfWork.StudentRepository.FindAllAsync(s =>
                    s.AccountStatus == Domain.Enums.AccountStatus.Active
                    && (
                ...
                var parentRelationships = await _unitOfWork.ParentStudentRepository.FindAllAsync(
                    ps => ps.ParentId == dto.ParentId && studentIds.Contains(ps.StudentId)
                );

                var relatedStudentIds = new HashSet<string>(
                    parentRelationships.Select(pr => pr.StudentId)
                );
            ...
            {
                return ServiceResult<List<StudentSearchResultDto>>.Fail(
