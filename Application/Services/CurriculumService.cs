...
namespace Application.Services
{
    public class CurriculumService : ICurriculumService
    {
        ...
        {
            var curricula = await _unitOfWork.CurriculumRepository.GetAllAsync();
            var dtos = _mapper.Map<IEnumerable<CurriculumDto>>(curricula);
            return ServiceResult<IEnumerable<CurriculumDto>>.Ok(
                dtos,
        ...
        public async Task<ServiceResult<CurriculumDto>> CreateAsync(CreateCurriculumDto dto)
        {
            try
            {
                ...
                var exists = await _unitOfWork.CurriculumRepository.ExistsByNameAsync(dto.Name);
                if (exists)
                    return ServiceResult<CurriculumDto>.Fail(
                        $"Curriculum with name '{dto.Name}' already exists."
                    );
                ...
                return ServiceResult<CurriculumDto>.Ok(
                    resultDto,
            ...
            {
                return ServiceResult<CurriculumDto>.Fail(
                    $"Error creating curriculum: {ex.Message}"
                );
            }
        }

        public async Task<ServiceResult<CurriculumDto>> UpdateAsync(
            string id,
            UpdateCurriculumDto dto
        )
        {
            try
            {
            ...
            {
                return ServiceResult<CurriculumDto>.Fail(
                    $"Error updating curriculum: {ex.Message}"
                );
            }
        }
            ...
            try
            {
                ...
                if (curriculum.Students?.Any() == true)
                    return ServiceResult<bool>.Fail(
                        "Cannot delete curriculum that has students assigned."
                    );

                if (curriculum.Teachers?.Any() == true)
                    return ServiceResult<bool>.Fail(
                        "Cannot delete curriculum that has teachers specialized."
                    );
        ...
        {
            var curriculum = await _unitOfWork.CurriculumRepository.GetWithDetailsAsync(id);
            if (curriculum == null)
                return ServiceResult<CurriculumDetailsDto>.Fail(
                    $"Curriculum with ID '{id}' not found."
                );

            var detailsDto = _mapper.Map<CurriculumDetailsDto>(curriculum);
            return ServiceResult<CurriculumDetailsDto>.Ok(
                detailsDto,
                "Curriculum details retrieved successfully"
            );
        }

        public async Task<ServiceResult<bool>> HasStudentsAsync(string curriculumId)
        {
            var curriculum = await _unitOfWork.CurriculumRepository.GetByIdAsync(curriculumId);
            if (curriculum == null)
            ...
            return ServiceResult<bool>.Ok(
                ...
                curriculum.Students?.Any() == true
                    ? "Curriculum has students"
                    : "Curriculum has no students"
            );
        }
        ...
        public async Task<ServiceResult<bool>> HasTeachersAsync(string curriculumId)
        {
            ...
            return ServiceResult<bool>.Ok(
                ...
                curriculum.Teachers?.Any() == true
                    ? "Curriculum has teachers"
                    : "Curriculum has no teachers"
            );
        }
        ...
        public async Task<ServiceResult<int>> GetCountAsync()
        {
            try
            {
                var allCurricula = await _unitOfWork.CurriculumRepository.GetAllAsync();
